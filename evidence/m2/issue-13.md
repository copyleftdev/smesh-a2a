...
- `authorized_gateway_process::production_mtls_gateway_migrates_restarts_and_serves_both_protocols` launches the production binary with required mTLS, a real client certificate handshake, the authorization policy, and SQLite. It proves an unbound legacy v1 database and a mismatched owner binding fail without changing version or task count; the enrolled tenant/owner migrates to v5 with the legacy task visible; authenticated JSON-RPC and REST send/get/list work; SIGTERM is reaped under a watchdog; and a fresh process reopens the exact database with tasks visible.
...
- Fresh v5 schema, ownership/child-scope triggers, append-only audit, scoped read/list audit, same-message isolation, and restart are directly covered.
...
- `atomic_lifecycle::exact_v1_schema_migrates_to_v5_with_explicit_binding_preserving_keys_and_task` and `malformed_v1_record_rolls_back_migration_without_version_or_schema_change` cover explicit legacy migration preservation and corruption rollback.
- `sqlite_store::tests::active_v4_outbox_migrates_with_preserved_legacy_dispatch_identity` covers an active historical-scope outbox row. A corrupt dispatch fails with the complete v4 schema/version intact; after repairing the fixture, migration preserves the legacy dispatch as identity version 1 and validates complete v5 semantics before commit.
