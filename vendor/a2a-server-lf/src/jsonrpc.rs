...
async fn handle_streaming_request<H: RequestHandler>(
    ...
    match request.method.as_str() {
        methods::SEND_STREAMING_MESSAGE => {
            match protojson_conv::from_value::<SendMessageRequest>(raw_params) {
                Ok(req) => match state.handler.send_streaming_message(params, req).await {
                    ...
                    Err(e) => streaming_error_response(id, e),
                },
                Err(e) => streaming_error_response(id, parse_error(e)),
            }
        }
        ...
        _ => streaming_error_response(id, A2AError::method_not_found(&request.method)),
    }
}
...
fn streaming_error_response(id: JsonRpcId, err: A2AError) -> axum::response::Response {
    // Streaming quota admission is a transport preflight boundary: callers must
...
mod tests {
    ...
    #[test]
    fn quota_http_status_is_non_200_only_for_streaming_preflight() {
        let error = A2AError::new(-32_010, "quota exceeded");
        assert_eq!(
        ...
        assert_eq!(
            error_response(JsonRpcId::Number(1), unavailable.clone()).status(),
            StatusCode::OK
        );
        assert_eq!(
            streaming_error_response(JsonRpcId::Number(1), unavailable).status(),
