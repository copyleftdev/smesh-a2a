...
test('operational CI runs an exact canary-free browser readiness diagnostic before fixtures', () => {
  ...
  assert.match(browserReadiness, /const INTERCEPTED_RESPONSE_HTML =/);
  assert.match(browserReadiness, /const directUrl = `http:\/\/127\.0\.0\.1:\$\{port\}\/direct`/);
  assert.match(browserReadiness, /const interceptedUrl = `http:\/\/127\.0\.0\.1:\$\{port\}\/intercepted`/);
  assert.match(browserReadiness, /DIRECT_LOOPBACK=DENIED/);
  assert.match(browserReadiness, /DIRECT_LOOPBACK=UNEXPECTED_/);
  assert.match(browserReadiness, /setRequestInterception\(true\)/);
  assert.match(browserReadiness, /request\.respond\(\{/);
  assert.match(browserReadiness, /body: INTERCEPTED_RESPONSE_HTML/);
  assert.doesNotMatch(browserReadiness, /request\.continue/);
  assert.match(browserReadiness, /page\.goto\(interceptedUrl/);
  assert.match(browserReadiness, /document\.querySelector\('#cdp-intercepted-ready'\)\?\.textContent/);
  assert.match(browserReadiness, /assert\.equal/);
  assert.match(browserReadiness, /interceptedSocketRequests, 0/);
  assert.match(browserReadiness, /INTERCEPTED_LOOPBACK=READY/);
  assert.match(browserReadiness, /INTERCEPTED_LOOPBACK=FAILED/);
  assert.match(browserReadiness, /browser_version=\$\{JSON\.stringify\(browserVersion\)\}/);
